 ...
 * Below the notes, a single "frame/key over time" lane shows the shipped spiral substrate's
 ...
 * badges) — this viz has exactly one substrate (the shipped spiral) so there is exactly one key lane.
 */
...
const ROW_H = 9;
const PAD = 8;
const LANE_GAP = 10;   // space between the note rows and the key lane
const LANE_H = 22;     // height of the key (frame) lane
const CAPTION_H = 16;  // space under the lane for the flip-count caption
...
export function renderPianoRoll(replay: Replay, step: number): void {
    if (replay !== builtForReplay) { build(replay); builtForReplay = replay; }
    updatePlayhead(replay, step);
}

function build(replay: Replay): void {
    ...
    const laneTop = PAD + noteH + LANE_GAP;
    const lanesBottom = laneTop + LANE_H;
    ...
    buildKeyLane(replay, svg, width, laneTop, height);
...
function buildKeyLane(replay: Replay, host: SVGSVGElement, width: number, laneTop: number, height: number): void {
    ...
    for (const seg of segs) {
        ...
        if (w > 42) text(x + w / 2, laneTop + LANE_H / 2 + 4, keyName(seg.lof), '#e6e8ec', 'middle');
    }
    ...
    text(PAD + 2, laneTop + 8, 'key', '#8b93a1', 'start', 8);
    ...
    text(PAD, height - PAD, `key: ${flips} change${flips === 1 ? '' : 's'} · ${rate} per 100 onsets`, '#9aa4b2');
}
