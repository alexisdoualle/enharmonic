 ...
 *
 * Scores this repo's four shipped modes against the held-out Meredith 8×25000 corpus (the real
...
for (const variant of done) {
    ...
    md.push('### The four modes (our shipped spellers)\n');
    md.push('| Mode | Latency | exact % | coherent % | wrong % |');
    md.push('|---|---|--:|--:|--:|');
    for (const [key, label, lat] of MODES) { const t = d.modes[key]; md.push(`| ${label} | ${lat} | ${ex(t)} | ${co(t)} | ${wr(t)} |`); }
    md.push('\n### Vs published baselines\n');
    md.push('| Algorithm | Latency | exact % | coherent % | wrong % | coverage % |');
    md.push('|---|---|--:|--:|--:|--:|');
    for (const [key, label, lat] of MODES) { const t = d.modes[key]; md.push(`| **${label}** | ${lat} | ${ex(t)} | ${co(t)} | ${wr(t)} | ${pct(t.committed, t.total)} |`); }
